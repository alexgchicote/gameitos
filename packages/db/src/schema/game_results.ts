...
export const gameResults = pgTable('game_results', {
  ...
  gameMatchId: uuid('game_match_id').references(() => matches.id).notNull(),
  playerId: uuid('player_id').references(() => players.id).notNull(),
  position: integer('position').notNull(),    // 1st, 2nd, 3rd, etc.
  pointsAwarded: integer('points_awarded').notNull(),
  positionFromMedian: integer('position_from_median').notNull(), // TODO: Make not null. Position relative to median: +3, +2, +1, 0, -1, -2, -3
